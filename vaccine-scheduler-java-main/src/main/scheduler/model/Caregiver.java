...
public class Caregiver {
    ...
    /***********************/
    public static class searchAvailability {
        private String username;
        private Date availabledates;

//        public searchAvailability(String username) {
//            this.username = username;
//        }

        public Caregiver get(Date d) throws SQLException {
            ConnectionManager cm = new ConnectionManager();
            Connection con = cm.createConnection();

            String searchAvailability = "SELECT Time, Username FROM Availabilities WHERE Time = ?";
            try {
                PreparedStatement statement = con.prepareStatement(searchAvailability);
                statement.setDate(1, d);
                ResultSet resultSet = statement.executeQuery();
                while (resultSet.next()) {
                    this.availabledates = resultSet.getDate("Time");
